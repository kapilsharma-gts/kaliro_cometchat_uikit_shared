...
class _CometChatSchedulerBubbleState extends State<CometChatSchedulerBubble> {
    ...
    if (widget.schedulerMessage.dateRangeStart != null && widget.schedulerMessage.dateRangeStart!.isNotEmpty) {
      ...
      if (widget.schedulerMessage.timezoneCode != null && widget.schedulerMessage.timezoneCode != timeZoneIdentifier) {
        from = SchedulerUtils.getConvertedTime(
            SchedulerUtils.getConvertedTime(from!, widget.schedulerMessage.timezoneCode!).toLocal(),
            timeZoneIdentifier);
      } else {
        from = from!.toLocal();
      }
    ...
    if (widget.schedulerMessage.dateRangeEnd != null && widget.schedulerMessage.dateRangeEnd!.isNotEmpty) {
      ...
      if (widget.schedulerMessage.timezoneCode != null && widget.schedulerMessage.timezoneCode != timeZoneIdentifier) {
        to = SchedulerUtils.getConvertedTime(
            SchedulerUtils.getConvertedTime(to!, widget.schedulerMessage.timezoneCode!).toLocal(), timeZoneIdentifier);
      } else {
        to = to!.toLocal();
      }
    } else {
    ...
    switch (currentStage) {
        ...
        title = Row(
          ...
          children: [
            CometChatAvatar(
              ...
              style: widget.schedulerBubbleStyle?.avatarStyle ?? const AvatarStyle(),
            ),
          ],
        );
        subtitle = Text(SchedulerUtils.getSchedulerTitle(widget.schedulerMessage, context),
      ...
      case SchedulerBubbleStage.initial:
        ...
        subtitle = Flexible(
            child: Container(
                ...
                child: Text(SchedulerUtils.getSchedulerTitle(widget.schedulerMessage, context),
                    textAlign: TextAlign.center,
      ...
      case SchedulerBubbleStage.timePicker:
        ...
        subtitle = Padding(
          ...
          child: getTextWithIcon("${meetingDuration.inMinutes} min", Icons.watch_later_outlined,
              theme.typography.title2.fontSize, theme.palette.getAccent500()),
        );
  ...
  Widget getMainContent() {
    ...
    BoxBorder? topBorder = Border(top: BorderSide(width: 1, color: theme.palette.getAccent200()));
    ...
    switch (currentStage) {
          ...
          child: Theme(
              data: Theme.of(context).copyWith(
                colorScheme: ColorScheme.light(
                  primary: widget.schedulerBubbleStyle?.calendarSelectedDayBackgroundColor ??
                      theme.palette.getPrimary(), // header background color
                  onPrimary: widget.schedulerBubbleStyle?.calendarSelectedDayTint ??
                      theme.palette.accent.dark, // header text color
                ),
                datePickerTheme: DatePickerThemeData(
                  weekdayStyle: widget.schedulerBubbleStyle?.calendarWeekdayTextStyle ??
                      TextStyle(color: theme.palette.getAccent()),
                ),
              ),
              child: CalendarDatePicker(
                initialDate: SchedulerUtils.nearestSelectableDate(
                    from ?? now, from, to, blockedDates, widget.schedulerMessage.availability),
                firstDate: from ?? now,
                lastDate: to ?? now,
                onDateChanged: (date) {
                ...
                selectableDayPredicate: (date) {
                  return SchedulerUtils.isDateSelectable(
                      date, from, to, blockedDates, widget.schedulerMessage.availability);
                },
              )),
        ));
      ...
      case SchedulerBubbleStage.timePicker:
        ...
        content.add(Container(
            ...
            width: double.infinity,
            decoration:
                BoxDecoration(border: Border(bottom: BorderSide(width: 1, color: theme.palette.getAccent100()))),
            child: getTextWithIcon(DateFormat('d MMMM y, EEEE').format(selectedDateTime ?? now),
                Icons.calendar_month_outlined, 16, null)));
        content.add(Padding(
      ...
      case SchedulerBubbleStage.goalCompleted:
        ...
        content.add(Container(
          ...
          decoration: BoxDecoration(color: theme.palette.getSecondary(), borderRadius: BorderRadius.circular(8)),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Text(
                        widget.schedulerMessage.goalCompletionText ?? Translations.of(context).goalAchievedSuccessfully,
                        style: TextStyle(
                                color: theme.palette.getAccent(),
                                fontWeight: theme.typography.body.fontWeight,
                                fontSize: theme.typography.body.fontSize,
                                fontFamily: theme.typography.body.fontFamily)
                            .merge(widget.schedulerBubbleStyle?.goalCompletionTextStyle),
                      ),
                    ),
                  ],
                ),
    ...
    switch (_scheduleStatus) {
      ...
      case ScheduleStatus.error:
        ...
        message = Padding(
          ...
          child:
              Text("${Translations.of(context).somethingWentWrongError}. ${Translations.of(context).pleaseTryAgain}.",
                  textAlign: TextAlign.center,
                  style: TextStyle(
      ...
      case ScheduleStatus.slotUnvailable:
        ...
        message = Padding(
          padding: const EdgeInsets.only(bottom: 6.18),
          child:
              Text("${Translations.of(context).yourSlotHasBeenBooked}. ${Translations.of(context).pleaseTryNewSlot}.",
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    color: theme.palette.getError(),
                    fontSize: theme.typography.caption2.fontSize,
      ...
      case ScheduleStatus.loading:
        loading = Image.asset(
          ...
          package: UIConstants.packageName,
          color:
              widget.schedulerBubbleStyle?.scheduleButtonStyle?.loadingIconTint ?? theme.palette.backGroundColor.light,
        );
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
                borderRadius:
                    BorderRadius.circular(widget.schedulerBubbleStyle?.scheduleButtonStyle?.borderRadius ?? 4),
            ...
            child: ElevatedButton(
                onPressed: () {
                  if (!buttonIsDisabled) {
                    if (widget.onScheduleClick != null) {
                      widget.onScheduleClick!(selectedDateTime ?? DateTime.now(), widget.schedulerMessage);
                    }
                    setState(() {
                ...
                style: ElevatedButton.styleFrom(
                    backgroundColor: buttonIsDisabled
                        ...
                        : widget.schedulerBubbleStyle?.scheduleButtonStyle?.background ?? theme.palette.getPrimary(),
  ...
  _scheduleMeeting() async {
    ...
    try {
      ...
      if (status == true) {
        markInteracted(widget.schedulerMessage);
      } else {
        setState(() {
          _scheduleStatus = ScheduleStatus.error;
        });
